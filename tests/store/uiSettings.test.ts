...
describe('UI theme lifecycle', () => {
  ...
  it('applies Mocha and Latte roots to the current document', () => {
    ...
    expect(document.documentElement.style.backgroundColor).toBe(
      normalizeColor(THEME_COLORS.mocha.background),
    );
    expect(document.documentElement.style.color).toBe(
      normalizeColor(THEME_COLORS.mocha.foreground),
    );

    applyDocumentTheme(false);
    expect(document.documentElement).not.toHaveClass('dark');
    expect(document.documentElement.style.backgroundColor).toBe(
      normalizeColor(THEME_COLORS.latte.background),
    );
    expect(document.documentElement.style.color).toBe(
      normalizeColor(THEME_COLORS.latte.foreground),
    );
  });
