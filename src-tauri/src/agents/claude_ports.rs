...
mod tests {
    ...
    fn skills_port_lists_and_plans_project_enable_without_writing() {
        ...
        let planned_source = plan.mutations[0]
            .content
            .as_ref()
            ...
            .unwrap();
        assert_eq!(
            std::fs::canonicalize(planned_source).unwrap(),
            std::fs::canonicalize(&skill).unwrap()
        );
        assert_eq!(
            plan.mutations[0].content.as_ref().unwrap()["digest"],
            serde_json::to_value(directory_tree_digest(&skill).unwrap()).unwrap()
        );
    ...
    #[test]
    #[serial_test::serial(home_env)]
    fn skills_port_disables_a_user_skill_from_a_project_context() {
        let (temp, mut context, _) = setup();
        let skill = temp.path().join(".ad/skill-library/source/demo");
        let global_link = temp.path().join(".claude/skills/demo");
        let project = temp.path().join("project");
        std::fs::create_dir_all(&skill).unwrap();
        std::fs::create_dir_all(global_link.parent().unwrap()).unwrap();
        std::fs::create_dir_all(&project).unwrap();
        std::fs::write(skill.join("SKILL.md"), "---\nname: demo\n---\n").unwrap();
        std::os::unix::fs::symlink(&skill, &global_link).unwrap();
        context.project_path = Some(
            std::fs::canonicalize(&project)
                .unwrap()
                .to_string_lossy()
        ...
        assert_eq!(demo.resource.scope, ResourceScope::User);
        let plan = port
            .plan_set_enabled(&context, &demo.resource, false)
            .unwrap();
    ...
    fn plugins_port_lists_and_plans_project_override_without_writing() {
        ...
        let plugins = port.list(&context).unwrap();
        let global = plugins
            .iter()
            .find(|snapshot| snapshot.resource.logical_id == "global")
            .unwrap();
        assert_eq!(global.location.origin, ResourceOrigin::User);
        assert_eq!(
            std::fs::canonicalize(&global.location.path).unwrap(),
            std::fs::canonicalize(temp.path().join(".claude/settings.json")).unwrap()
        );
        let plugin = plugins
            .into_iter()
            .find(|snapshot| snapshot.resource.logical_id == "demo")
            .unwrap();
        let override_target = port.resolve(&context, &plugin.resource).unwrap();
        assert_eq!(
            override_target.path().parent().unwrap(),
            std::fs::canonicalize(project.join(".claude")).unwrap()
        );
        assert_eq!(
            override_target.path().file_name().unwrap(),
            "settings.local.json"
        );
        let plan = port
