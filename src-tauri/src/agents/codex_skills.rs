...
use super::codex_ports::{agent_error, read_optional, resolve_codex_home, validate_project_path};
use super::{
...
fn plan_skill_config(
    ...
    configs.retain(|entry| entry.get("path").and_then(toml::Value::as_str) != Some(path.as_str()));
    if !enabled {
    ...
    Ok(config_plan(context, resource, rendered, expected_digest))
}
