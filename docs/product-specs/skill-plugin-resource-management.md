# Product Contract: Harness 中的 Skill / Plugin 管理与项目安装

Authority: 用户于 2026-08-13 确认的资源管理决策，以及 2026-08-15 确认的 Harness 产品定位

Product Context: `docs/product-specs/project-agent-workspace.md`

## Actor and observable outcome

AD 用户可以从与“项目”并列的顶级 Harness 工作区进入 Skills / Plugins 子功能，集中管理来自 Git 仓库或本地目录的 Skill 和 Plugin，然后在具体项目中从该受管库选择资源及目标 Agent（Codex 或 Claude Code）完成安装和卸载。Harness 是面向 Agent 的可复用能力管理工作区；本期只开放 Skills / Plugins 子功能。该子功能中的受管库是 AD 安装 Skill / Plugin 的唯一资源池，未进入受管库的内容不能通过 AD 安装。

安装后的资源内容始终关联 Harness 受管库中的原始或受管 source checkout；库中内容更新后，已安装项目无需重新安装即可使用新内容。AD 屏蔽 Agent 的安装机制差异，但不转换、改写、裁剪或重新打包 Skill / Plugin 原始内容。任何从项目内发起的操作只影响当前项目和用户选择的 Agent。

## Requirements

- **R1 — Harness 顶级工作区与内部能力导航。** AD 提供与“项目”并列的顶级 Harness 入口，将其定位为面向 Agent 的可复用能力工作区。Harness 内部提供能力导航，本期 Skills / Plugins 是唯一可用子功能；该子功能以 Skill / Plugin 资源为主体展示已导入内容，负责 source 导入、扫描、更新和删除。MCP 可以作为不可用的未来能力位置显示，但不得提供入口、配置或已支持暗示。Harness 不是资源市场，不承担发现或推荐。
  - Acceptance: 桌面产品顶级导航显示 Harness；进入后一级标题为 Harness，内部能力导航显示当前可用的 Skills / Plugins；MCP 显示为禁用且带“即将推出”状态，不能进入或执行任何动作；资源视图围绕资源而不是 source 配置表；界面中不出现市场搜索、推荐或远程发现承诺。
  - Owner / method: Product / Design 通过真实桌面界面走查验收导航、信息层级和能力边界。
  - Provenance: 用户于 2026-08-15 明确要求将顶级“资源”升级为 Harness、将 Skills / Plugins 管理作为子功能，并确认 Harness 给 Agent 使用、采用 Harness 内部能力导航且先预留 MCP 位置。

- **R2 — 多来源纳管与可辨识选择。** 本期 source 支持 Git 仓库和本地目录。不同 source 中同名的资源是独立候选项，不在 Harness 受管库中判定为冲突；用户在安装选择时必须能看到来源类型并辨别具体来源。
...
- **R4 — 项目与 Agent 精确安装。** 安装入口位于具体项目。用户选择资源后，必须选择 Codex 或 Claude Code；安装、启用、禁用和卸载只能改变当前项目与所选 Agent。AD 对用户提供统一产品语义，不要求用户理解 Agent 的安装位置或注册差异。
  - Acceptance: 项目安装候选只来自 Harness 的 Skills / Plugins 受管库；在项目 A 为 Codex 安装资源后，项目 A 的 Claude Code 和项目 B 的两个 Agent 状态与文件均不变；卸载具有同样的隔离性。
...
- **R6 — 受管库更新自动传播。** Skill 与 Plugin 的原始文件必须通过软链接指向 Harness 受管库中的原始或受管 source checkout。如果 Agent 还要求启用、注册或其他必要配置，AD 可以只在当前项目的所选 Agent 上管理这些附加状态，卸载时一并清理。
...
- **R8 — 非 AD 托管资源只展示、不接管。** 如果用户通过其他途径在项目/Agent 中安装了 Skill / Plugin，AD 应在项目资源清单中将其标识为“非 AD 托管”。如果它占用了 AD 资源的安装目标，AD 只说明占用状态并允许用户重新检查。AD 不把它自动加入 Harness 受管库，不提供安装来源推断，也不删除、卸载、备份、恢复或接管该资源。
  - Acceptance: 项目清单能展示 Agent 中已存在但无 AD ownership 的 Skill / Plugin，并明确标记为“非 AD 托管”；该资源不出现在 Harness 受管库中；其操作区不提供 AD 卸载、删除、替换、移除并备份、恢复或接管动作；检查前后外部项字节与路径不变。
...
- **R10 — 移除单个受管资源前完成安装清理。** 用户可以从 Harness 受管库移除某一个 Skill 或 Plugin，而不影响同一 source 中的其他资源。该资源仍被项目安装时，移除前必须告知受影响的项目数和 Agent 安装数；用户确认后，AD 对每个受管安装调用与项目页一致的标准卸载动作，并展示整体及逐项进度。所有必要卸载完成后才将该资源移出受管库，不修改 Git 仓库或本地目录，也不留下悬空链接。
  - Acceptance: 单个资源移除确认清楚呈现影响数量；取消时零变更；确认后各项目/Agent 按标准卸载规则清理并展示进度；任一项失败时逐项呈现失败，不报告整体成功，且资源保持可管理以便重试未完成项；成功后同一 source 中其他资源仍在 Harness 受管库，source 原始内容不变。
  - Owner / method: Engineering 通过多项目资源移除、取消和部分失败场景验收；Product 审核破坏性确认的信息充分性。
  - Provenance: 用户要求删除前告知所有已安装项目将失效，并确认删除时按各项目的卸载方式清理。

- **R11 — 已移除资源必须先重新加入受管库。** 单个 Skill / Plugin 被移出 Harness 受管库后，source 刷新、内容更新、应用重启或项目扫描都不得将它自动恢复为受管资源。用户若要再次安装，必须先在 Harness 的 Skills / Plugins 子功能中主动重新添加该资源；资源重新进入受管库后，才可以从具体项目选择 Agent 发起安装。“重新添加资源”和“安装到项目”是两个明确动作。
  - Acceptance: 移除后的资源不出现在项目的 AD 安装候选中；刷新 source、更新内容、重启 AD 和重新扫描项目后仍不恢复；用户在 Harness 中主动重新添加后，它重新出现在受管库和项目安装候选中，但在用户于具体项目发起安装前不会写入任何项目/Agent。
...
- **R12 — 界面是产品化的管理体验。** Harness 应先表达 Agent 能力工作区定位，其 Skills / Plugins 子功能呈现为有策展感的资源库；项目页聚焦“已安装”、“可安装”和当前最主要的下一步操作。信息通过层级、状态和按需展开呈现，不把机制说明和所有诊断信息平铺在主界面。
...
- 一个与项目并列的顶级 Harness 工作区、内部能力导航，以及其中的 Skills / Plugins 管理子功能。
- MCP 的禁用导航位置及“即将推出”状态，不包含任何 MCP 管理行为。
...
- Harness、Skills / Plugins 子功能和项目安装界面的产品化信息架构、状态与视觉验收。
...
- MCP 管理及其他 Harness 子功能；本期仅允许展示不可交互的 MCP 未来能力位置。
- Skill / Plugin 内容转换、跨 Agent 改写、兼容层注入、裁剪或重新打包。
- 为不支持的 Agent 伪造降级版 Plugin / Skill。
- 删除、备份、恢复、接管或自动修复 external/unowned 项。
- 从 Harness 发起跨项目安装、绕过 Harness 受管库的 AD 安装，以及 user/global Agent 安装。
- Agent 的具体文件位置、注册格式、配置载体与适配算法；这些由后续技术设计确定。

## Constraints and confirmed decisions

- AD 的长期产品定性是以项目为作用域：项目中的操作不得影响其他项目。
- Harness 是面向 Agent 的全局能力管理面，但不提供跨项目安装；本期仅包含 Skills / Plugins 子功能，MCP 仅为禁用的未来能力标识。“移除中心资源”是用户在明确影响范围后发起的全局生命周期操作，不属于项目操作。
- Harness 中的 Skills / Plugins 受管库是 AD 安装 Skill / Plugin 的唯一资源池；项目安装不能自行发现或临时引入资源。
...
- 非 AD 安装的 Skill / Plugin 只在项目中标识为“非 AD 托管”，不会自动进入 Harness 受管库或变为 AD-owned。
...
- 工程可自主选择如何持久表达“source 中该资源已被用户移除”，但该状态必须跨刷新、内容更新、重启和项目扫描保持稳定，并只允许由 Harness 中的主动重新添加动作解除。
...
- Harness 的 MCP 管理子功能；当前只有禁用导航位置，正式纳入时需单独确认 MCP 的来源、作用域、安装/启用和兼容性语义。
