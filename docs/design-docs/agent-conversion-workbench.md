...
- `.claude` 中 Settings、Skills、Plugins 和 marketplace 等不同配置载体被混在一个扁平 artifact 列表里；只有 settings mutation 会进入计划。
...
新设计把转换定义为一个“配置工作台”：先完整盘点源配置，再按目标载体路由，每个源 artifact 必须得到一个可解释、可操作的最终状态。`config.toml` 只是 Settings 载体之一；Project Skills 写入 `.agents/skills`，Plugins 进入 marketplace/授权后续设置，不把它们伪装成 TOML 字段。
...
Claude source inventory
  ...
  ├─ Plugins / marketplaces ─────────────→ Codex plugin marketplace + setup status
...
| 6 Plugins / 2 marketplaces | Plugin setup | 修复项目声明盘点；可确认等价身份时导入，否则给出逐项 marketplace/授权后续设置 |
...
- Codex Plugin 安装涉及 marketplace、host 和授权。没有受支持的安全 API 时，转换项应为“需要完成设置”或 `unsupported`，并引导用户到 Codex `/plugins`，不能向 `config.toml` 写一个虚构 plugin table。
- Skill 的本地目录/symlink source 可由两个 adapter 安全解析，目标 Port 已支持 Project `.agents/skills`，因此应纳入真实 mutation plan。
- Plugin 后续设置不应阻塞 Settings/Skills 的安全写入；receipt 必须保留未完成项目摘要。
...
5. Project-only Plugin 不遗漏；无法自动安装时逐项说明 marketplace/授权后续动作。
