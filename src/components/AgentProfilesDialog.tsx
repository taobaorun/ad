...
function AgentProfilesDialog({
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const next = await tauri.listProfileEnvelopes(requestAgentId);
      if (requestId !== loadRequestRef.current || requestAgentId !== activeAgentIdRef.current) {
        return;
      }
      setProfiles(next);
  ...
  async function saveProfile() {
    ...
    try {
      ...
      const saved = await tauri.saveProfileEnvelope(withPayload);
      setProfiles((current) =>
  ...
  async function deleteProfile() {
    ...
    if (
      !deleting ||
      !window.confirm(t('agentProfiles.deleteConfirm', { name: deleting.metadata.displayName }))
    )
      return;
    setBusy(true);
    setError(null);
    try {
      await tauri.deleteProfileEnvelope(deleting.key.agentId, deleting.key.profileId);
      if (activeAgentIdRef.current !== deleting.key.agentId) return;
      const remaining = profiles.filter(
        (profile) =>
          profile.key.agentId !== deleting.key.agentId ||
          profile.key.profileId !== deleting.key.profileId,
      );
